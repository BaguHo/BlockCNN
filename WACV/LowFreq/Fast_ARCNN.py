    ...
    for original, denoised in train_loader:
        ...
        if output.size() != original.size():
            output = torch.nn.functional.interpolate(
                output, size=original.shape[2:], mode='bilinear', align_corners=False)
        ...
        for original_val, denoised_val in val_loader:
            ...
            if outputs_val.size() != original_val.size():
                outputs_val = torch.nn.functional.interpolate(
                    outputs_val, size=original_val.shape[2:], mode='bilinear', align_corners=False)
...
with torch.no_grad():
    for original_test, denoised_test in test_loader:
        ...
        if outputs_test.size() != original_test.size():
            outputs_test = torch.nn.functional.interpolate(
                outputs_test, size=original_test.shape[2:], mode='bilinear', align_corners=False)
